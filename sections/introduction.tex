...
the optimization framework in {\tt ensmallen} is generic and flexible.
Numerous function types are supported:
arbitrary, differentiable, separable, categorical, constrained, and semidefinite.
Furthermore, custom behavior during optimization can be specified via {\it callbacks}.
Lastly, the framework facilitates the implementation of new optimization techniques.
As such, the framework supports a wide range of use cases and applications.
